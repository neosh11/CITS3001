...
public class MossSideWhist{
	...
	private static String leader; //the name of the first player
	private static String left; //the name of the second player
	private static String right; //the name of the third player
	private Map<String, MSWAgent> agents;  //a map from names to players
	private static Map<String, Integer> scoreboard;  //a map from names to scores
	...
	{
		if((scoreboard.get(leader)> scoreboard.get(left)) && (scoreboard.get(leader)> scoreboard.get(right)))
		{
			wins[0]++;
		}
		else if((scoreboard.get(left)> scoreboard.get(right)) && (scoreboard.get(left)> scoreboard.get(leader)))
		{
			wins[1]++;
		}
		else if((scoreboard.get(right)> scoreboard.get(left)) && (scoreboard.get(right)> scoreboard.get(leader)))
		{
			wins[2]++;
		}
	}
