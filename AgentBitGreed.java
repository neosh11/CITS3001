 ...
 *
 */
public class AgentBitGreed implements MSWAgent  {
	...
	/**
	 * Prints cards in the current hand
	 */
...
class View
{
	...
	{
		hand[suit] = ((hand[suit]&(~(1<< value))) & AgentBitGreed.THIRTEEN);
	...
	void addToGrave(int suit, int card)
	{
		...
		for(int i =0; i < 4; i++)
		{
			if((grave[i]|hand[i]) == AgentBitGreed.THIRTEEN)
			{
				otherSuits[1][i] = true;
				otherSuits[0][i] = true;
			}
	...
	{
		return AgentBitGreed.binlog(Integer.highestOneBit(i));
	}
